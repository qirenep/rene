...
token = "LX1hRWzTShjrIFbI001hK3GTpXjwokOn"
main_color=0xfa9c1e
...
class Statistics(commands.Cog):
    ...
    async def get_patch_notes(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(self.configpatch) as r:
                content = await r.read()
                page = BeautifulSoup(content, features="html.parser")
    ...
    async def news(self, ctx):
        ...
        async with ctx.typing():
            pages = []
            try:
                title, link, desc, img = await self.get_news()
            except Exception:
                embed = discord.Embed(
                    title="Latest Overwatch News",
                    description=f"[Click here to check out all the new Overwatch news.](https://playoverwatch.com/ko-kr/news/pc)",
                    color=main_color
                )
    ...
    async def patch(self, ctx):
        ...
        async with ctx.typing():
            try:
                title = await self.get_patch_notes()
            except Exception:
...
bot.run('NjEyOTMxMTg4ODU1OTMwODkw.XkrUoQ.KK6ooQK-lUxjCa1rlIRagQxD_fM')
